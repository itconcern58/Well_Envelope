...
@Component({
  selector: 'app-modal-dd1hr-next',
  templateUrl: './modal-dd1hr-next.component.html',
  styleUrls: ['./modal-dd1hr-next.component.scss']
})
export class ModalDd1hrNextComponent implements OnInit {
  ...
  number4: string = '1';
  number5: string = '1';
  resultDD1hrNext: number;

  constructor(
    public dialogRef: MatDialogRef<ModalDd1hrNextComponent>,
  ...
  calcDD1hrNext() {
    this.resultDD1hrNext = Number((this.Pwf1hr-this.pressData[0]).toFixed(2));

    const resultDD1hrnext = this.Pwf1hr-this.pressData[0];
    const signPostFlagDD1hrnext = 0.75*( this.Pwf1hr-this.pressData[0]);

    this.router.navigate(['completion-envelope'], { state: { resultDD1hrnext } });
    this.router.navigate(['completion-envelope'], { state: { signPostFlagDD1hrnext } });
    // or
    this.sharedService.storeDd1hrnextValue = Math.round(resultDD1hrnext);
    this.sharedService.signPostFlagDd1hrnextValue = Math.round(signPostFlagDD1hrnext);
    }
