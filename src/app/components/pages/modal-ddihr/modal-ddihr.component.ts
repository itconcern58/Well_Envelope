...
@Component({
  selector: 'app-modal-ddihr',
  templateUrl: './modal-ddihr.component.html',
  styleUrls: ['./modal-ddihr.component.scss']
})
export class ModalDdihrComponent implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<ModalDdihrComponent>,
  ...
  calcDD1hrCurr() {
    this.resultDDihr = Number((this.Pwf1hr-this.pressData[0]).toFixed(2));

    const resultDD1hrcurr = this.Pwf1hr-this.pressData[0];
    const signPostFlagDD1hrcurr = 0.75*( this.Pwf1hr-this.pressData[0]);

    this.router.navigate(['completion-envelope'], { state: { resultDD1hrcurr } });
    this.router.navigate(['completion-envelope'], { state: { signPostFlagDD1hrcurr } });
    // or
    this.sharedService.storeDd1hrcurrValue = Math.round(resultDD1hrcurr);
    this.sharedService.signPostFlagDd1hrcurrValue = Math.round(signPostFlagDD1hrcurr);
    }
