...
@Component({
  selector: 'app-modal-kh-calc',
  templateUrl: './modal-kh-calc.component.html',
  styleUrls: ['./modal-kh-calc.component.scss']
})
export class ModalKHCalcComponent implements OnInit {
  ...
  resultKHCalc: number = 0;

  constructor(
    public dialogRef: MatDialogRef<ModalKHCalcComponent>,
...
calcKHnext() {
  ...
  calcKHCalc() {
    this.resultKHCalc = Number((162.6*parseFloat(this.number1)*parseFloat(this.number2)*parseFloat(this.number3)/Math.abs(this.currentGradient)).toFixed(2));
  }
