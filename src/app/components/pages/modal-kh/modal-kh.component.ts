...
@Component({
  selector: 'app-modal-kh',
  templateUrl: './modal-kh.component.html',
  styleUrls: ['./modal-kh.component.scss']
})
export class ModalKhComponent implements OnInit {
  ...
  number9: string = '1';
  number10: string = '1';
  resultKHcurr: number = 0;
  resultKHnext: number = 0;

  constructor(
    public dialogRef: MatDialogRef<ModalKhComponent>,
...
currentyIntercept = this.currentResult.equation[1];
 ...
 /*---Calculation of DD1hr ------*/
  ...
  calcKHcurr() {
    this.resultKHcurr = Number((162.6*parseFloat(this.number1)*parseFloat(this.number2)*parseFloat(this.number3)/Math.abs(this.currentGradient)).toFixed(2));

    const resultKhcurr = 162.6*parseFloat(this.number1)*parseFloat(this.number2)*parseFloat(this.number3)/Math.abs(this.currentGradient);
    this.router.navigate(['completion-envelope'], { state: { resultKhcurr } });
    // or
    this.sharedService.storeKhcurrValue = Math.round(resultKhcurr);
  };
...
nextyIntercept = this.nextResult.equation[1];
 ...
 /*---Calculation of DD1hr ------*/
  ...
  resultKHCalcnext =  Number((162.6*parseFloat(this.number1)*parseFloat(this.number2)*parseFloat(this.number3)/Math.abs(this.nextGradient)).toFixed(2));
  resultKHCalccurr =  Number((162.6*parseFloat(this.number1)*parseFloat(this.number2)*parseFloat(this.number3)/Math.abs(this.currentGradient)).toFixed(2));

calcKHnext() {
  this.resultKHnext = Number((162.6*parseFloat(this.number1)*parseFloat(this.number2)*parseFloat(this.number3)/Math.abs(this.currentGradient)).toFixed(2));

  const resultKhnext = 162.6*parseFloat(this.number1)*parseFloat(this.number2)*parseFloat(this.number3)/Math.abs(this.currentGradient);
  this.router.navigate(['completion-envelope'], { state: { resultKhnext } });
  // or
  this.sharedService.storeKhnextValue = Math.round(resultKhnext);
}

  calcKH() {
    if (this.resultKHCalccurr-this.resultKHCalcnext <= 1) {
      this.sharedService.storeKHDiffValue = "Constant"
      } else if ((this.resultKHCalccurr-this.resultKHCalcnext > 1) && (this.resultKHCalccurr-this.resultKHCalcnext < 2.0)) {
      this.sharedService.storeKHDiffValue = "Slightly Decreasing";
    } else {
      this.sharedService.storeKHDiffValue = "Rapid Decreasing";
    };
  }
