...
@Component({
  selector: 'app-modal-pi',
  templateUrl: './modal-pi.component.html',
  styleUrls: ['./modal-pi.component.scss']
})
export class ModalPiComponent implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<ModalPiComponent>,
...
currentyIntercept = this.currentResult.equation[1];
 ...
 /*---Calculation of DD1hr ------*/
  ...
  calcPICurr() {
    this.resultPIcurr = Number((parseFloat(this.number1)/(this.currentyIntercept-this.currentPressureData[0])));

    const resultPicurr = parseFloat(this.number1)/(this.currentyIntercept-this.currentPressureData[0]);
    this.router.navigate(['completion-envelope'], { state: { resultPicurr } });
    // or
    this.sharedService.storePicurrValue = Math.round(resultPicurr);
    }
...
/**---------------------------- */
  ...
  nextyIntercept = this.nextResult.equation[1];
   ...
   /*---Calculation of DD1hr ------*/
    ...
    resultPICalcnext = Number((parseFloat(this.number1)/(this.nextyIntercept-this.nextPressureData[0])));
    resultPICalccurr = Number((parseFloat(this.number1)/(this.currentyIntercept-this.currentPressureData[0])));
  ...
  calcPInext() {
    ...
    calcPI() {
      if (this.resultPICalccurr-this.resultPICalcnext <= 0.1) {
        this.sharedService.storeDiffPiValue = "Constant"
        } else if ((this.resultPICalccurr-this.resultPICalcnext > 0.1) && (this.resultPICalccurr-this.resultPICalcnext < 2.0)) {
        this.sharedService.storeDiffPiValue = "Slightly Decreasing";
      } else {
        this.sharedService.storeDiffPiValue = "Decreasing";
      };
    }
