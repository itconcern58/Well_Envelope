...
@Component({
  selector: 'app-modal-sk-calc',
  templateUrl: './modal-sk-calc.component.html',
  styleUrls: ['./modal-sk-calc.component.scss']
})
export class ModalSKCalcComponent implements OnInit {
 ...
 // resultKH: number;
  ...
  constructor(
    public dialogRef: MatDialogRef<ModalSKCalcComponent>,
...
calcSKnext() {
 ...
 const resultSKnext = 1.151*(((this.Pwf1hr-this.nextPressureData[0])/Math.abs(this.nextGradient))-Math.log(((162.6*parseFloat(this.number1)*parseFloat(this.number2)*parseFloat(this.number3))/
  ...
  calcSKCalc() {
    this.resultSKCalc = Number((1.151*(((this.Pwf1hr-this.currentPressureData[0])/Math.abs(this.currentGradient))-Math.log(((162.6*parseFloat(this.number1)*parseFloat(this.number2)*parseFloat(this.number3))/
    Math.abs(this.currentGradient)*parseFloat(this.number7))/(parseFloat(this.number4)*parseFloat(this.number1)*parseFloat(this.number5)*Math.pow(parseFloat(this.number6),2)) + 3.23))).toFixed(2));
  }
